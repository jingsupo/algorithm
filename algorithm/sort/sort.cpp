...
void sort::build_heap(std::vector<int>& vec, int start, int end)
{
    ...
    while (child < end)
    {
        if ((child + 1 < end) and (vec[child] < vec[child + 1]))//查看左右子树的最大节点
        {
            child++;
        }
...
void sort::quick(int left, int right)
{
    ...
    while (i < j)
    {
        while ((i < j) and (vec[j] > pivot))//从右向左找比pivot小的值
        {
            j--;
        }
        ...
        while ((i < j) and (vec[i] < pivot))//从左向右找比pivot大的值
        {
            i++;
        }
...
void sort::_merge(int left, int right, int mid)
{
    ...
    {
        if (vec[i] <= vec[j])//比较左右两个数组的元素大小，较小的元素放入tmp数组
        {
            tmp[idx++] = vec[i++];
        }
        else
    ...
    while (i <= mid)
    {
        tmp[idx++] = vec[i++];//将剩余的元素放入tmp数组
    }

    while (j <= mid)
    {
        tmp[idx++] = vec[j++];//将剩余的元素放入tmp数组
    }
